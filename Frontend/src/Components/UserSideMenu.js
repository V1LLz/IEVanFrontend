...
const UserSideMenu = ({ open, togglePanel }) => {
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 bg-black bg-opacity-50 z-[1090]"
          onClick={togglePanel} // Al hacer clic en el fondo, se cierra el menú
        />
      )}

      <div
        className={`fixed top-0 right-0 h-full w-80 bg-white text-white shadow-lg transform transition-transform duration-300 z-[1090] ${
